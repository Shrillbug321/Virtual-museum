...
public class PDFService {
    ...
    private void createPage(Document document, Painting painting, int pageNumber) throws IOException {
        ...
        document.add(new Paragraph("Data wygenerowania PDF-a: " + LocalDate.now()).setFontSize(10));
        document.add(new Paragraph("Data dodania obrazu: " + painting.getAddDate().toString()).setFontSize(10));
        ...
        table.addCell(getCell("Wartość [zł]")).addCell(getCell(Float.toString(painting.getValue()).replace('.', ',')));
        table.addCell(getCell("Wystawiony ")).addCell(getCell(painting.isExhibited() ? "tak" : "nie"));
        table.addCell(getCell("Technika ")).addCell(getCell(painting.getTechnique().getName()));
        table.addCell(getCell("Wymiary [cm]")).addCell(getCell(painting.getDimensions()));
        String genres = "";
    ...
    public void openPdfFile(@RequestParam String name, HttpServletResponse response) throws IOException {
        ...
        FileSystemResource pdfFile = new FileSystemResource(pdfsLocation + "/" + name + ".pdf");
